...
namespace TicTacToeServer
{
    public partial class Form1 : Form
    {
        ...
        private void StartGame()
        {
            ...
            List<string> players = Clients.Keys.ToList().GetRange(0,2);
            ...
            try
            {
                ...
                {
                    Byte[] gameinfobuffer;
                    string gameinfo = $":10:--------------\nGame Starts | {players[0]} (X player): {(ClientStatistics[players[0]]).GetValue(0)}/{(ClientStatistics[players[0]]).GetValue(1)}/{(ClientStatistics[players[0]]).GetValue(2)} vs. {players[1]} (O player): {(ClientStatistics[players[1]]).GetValue(0)}/{(ClientStatistics[players[1]]).GetValue(1)}/{(ClientStatistics[players[1]]).GetValue(2)}\n--------------\n";
                                ...
                                if (counter != 2)
                                {
                                    ...
                                    for (int i = 0; i < players.Count; i++)
                                    {
                                        ...
                                        { //Update list index of players 
                                            if (Clients.Keys.ElementAt(0) == players[Math.Abs(i-1)])
                                            {
                                                players[i] = Clients.Keys.ElementAt(1);
                                            }
                                            else
                                            {
                                                players[i] = Clients.Keys.ElementAt(0);
                                            }
                                        }
                                    }
